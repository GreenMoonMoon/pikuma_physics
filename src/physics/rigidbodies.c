...
static Body get_default_body(void) {
    Body result = {
        .rotation = 0.0f,
        .linear_velocity = {0},
        .angular_velocity = 0.0f,
        .restitution = 0.1f, // default
        .friction = 0.7f, // default
        .bounding_square = {0},
        .is_static = false,
        .is_colliding = false,
...
Body create_circle_body(const float radius, const float mass, const Vector2 position, const bool is_static) {
    ...
    Body body = get_default_body();
    body.position = position;
    body.inverse_mass = inverse_mass;
    body.mass = mass;
    body.angular_mass = angular_mass;
    body.inverse_angular_mass = inverse_angular_mass;
    body.type = CIRCLE_SHAPE_TYPE;
    body.circle_shape = (CircleShape){.radius = radius};
    body.bounding_square.min = Vector2Subtract(position, (Vector2){radius, radius});
    body.bounding_square.max = Vector2Add(position, (Vector2){radius, radius});
    body.is_static = is_static;

    return body;
}

Body create_box_body(const Vector2 center, const Vector2 extents, const float mass, const Vector2 position, const bool is_static) {
    ...
    body.inverse_angular_mass = inverse_angular_mass;
    body.is_static = is_static;
    body.type = POLYGON_SHAPE_TYPE;

    body.polygon_shape.vertices = malloc(sizeof(Vector2) * 8);
    body.polygon_shape.tfmd_vertices = &body.polygon_shape.vertices[4];
    if (body.polygon_shape.vertices != NULL) {
        body.polygon_shape.vertex_count = 4;
        body.polygon_shape.vertices[0] = (Vector2){center.x + extents.x, center.y + extents.y};
        body.polygon_shape.vertices[1] = (Vector2){center.x - extents.x, center.y + extents.y};
        body.polygon_shape.vertices[2] = (Vector2){center.x - extents.x, center.y - extents.y};
        body.polygon_shape.vertices[3] = (Vector2){center.x + extents.x, center.y - extents.y};

        update_polygon_shape(&body.polygon_shape, body.position, body.rotation);
    }

    return body;
}

Body create_polygon_body(const Vector2 *vertices, const int vertex_count, const float mass, const Vector2 position, const bool is_static) {
    float inverse_mass = 0.0f;
    if (!FloatEquals(mass, 0.0f)) {
        inverse_mass = 1.0f / mass;
    };
    ...
    body.is_static = is_static;
    body.type = POLYGON_SHAPE_TYPE;
    body.polygon_shape = (PolygonShape){
        .vertices = malloc(sizeof(Vector2) * vertex_count * 2),
        .vertex_count = vertex_count,
    };
    body.polygon_shape.tfmd_vertices = &body.polygon_shape.vertices[vertex_count];
    memcpy(body.polygon_shape.vertices, vertices, sizeof(Vector2) * vertex_count);
    update_polygon_shape(&body.polygon_shape, body.position, body.rotation);
...
void body_apply_impulse(Body* body, const Vector2 impulse, const Vector2 r) {
    ...
    // apply angular impulse ( j X r) along the normal
